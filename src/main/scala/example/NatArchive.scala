...
object NatArchive extends Messages with App {
  ...
  def updateRowValues(csvFileName: String, column: String, oldValue: String, newValue: String): Unit = {
    ...
    sink.flush
    sink.close
    // what should happen here, but is not done, to maintain visibility of the change, is to rename the 
    // 'corrected' file to the original filename
  }
...
trait Messages {  // artefact left over from giterate template.
  lazy val greeting: String = "Welcome to the National Archive file data modifier."
}
